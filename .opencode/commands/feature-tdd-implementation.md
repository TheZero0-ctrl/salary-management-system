...
8. **Handoff (No Commit)** -- Stop after verification and provide:
...
Use TDD-focused agents for each phase. Do not implement everything directly.

1. **RED phase** -- Use `rspec-agent` to write failing specs first.
2. **GREEN phase** -- Use `implementation-agent` to implement the minimum code needed to pass.
3. **REFACTOR phase** -- Use `tdd-refactoring-agent` to improve structure while keeping tests green.

For domain-specific implementation in GREEN, use the relevant specialist agent when needed:
- Models/validations/associations: `model-agent`
- Controllers/endpoints: `controller-agent`
- Business logic/services: `service-agent`
- Query objects/reporting: `query-agent`
- Jobs/async work: `job-agent`
- Policies/authorization: `policy-agent`
- Migrations/schema changes: `migration-agent`

Agent sequence per slice:
- `rspec-agent` -> run failing spec -> `implementation-agent` (or specialist) -> run passing spec -> `tdd-refactoring-agent` -> run full related specs -> handoff.
...
6. Always prefer the relevant TDD/specialist agent for each phase
